...
| `sync-commands.sh [--global] [--dry-run] [--yes]` | Diff + bulk `PUT` of commands to `DISCORD_DEV_GUILD_ID` (default) or `--guild <DISCORD_PROD_GUILD_ID>` / globally | After editing `commands.py`. Prod is never the implicit target |
